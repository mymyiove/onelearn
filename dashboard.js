...
function renderTenant() {
  els.tenantName.textContent = tenant.displayName || tenant.tenantName || "OneLearn";
  els.heroTitle.textContent = `${user?.name ? `${user.name}님, ` : ""}나에게 배정된 교육`;
  els.heroSubtitle.textContent =
    tenant.brand?.welcomeMessage ||
    "필수 교육, 진행 중 교육, 수료 완료 교육을 한 곳에서 확인하세요.";

  document.documentElement.style.setProperty("--ol-orange", tenant.brand?.primaryColor || "#F47721");
}
  ...
  courses.forEach((course) => {
    ...
    card.innerHTML = `
      <div class="course-meta">
        ${course.required ? `<span class="course-pill required">필수</span>` : `<span class="course-pill">선택</span>`}
        <span class="course-pill">${course.category || "교육"}</span>
        <span class="course-pill ${course.status === "completed" ? "completed" : ""}">${statusText}</span>
        ${course.urgent && course.status !== "completed" ? `<span class="course-pill urgent">마감 임박</span>` : ""}
      </div>
      ...
      <div class="course-info-grid">
        ...
        <div class="course-info">
          <span>D-Day</span>
          <strong>${getDdayText(course.dueDate)}</strong>
        </div>
        <div class="course-info">
          <span>챕터</span>
...
init().catch((error) => {
  ...
  els.courseGrid.innerHTML = `
    <section class="empty-state">
      <p class="dash-eyebrow">Load Error</p>
      <h3>교육 목록을 불러오지 못했습니다.</h3>
      <p>${error.message}</p>
    </section>
  `;
});
