...
const els = {
  ...
  learningLogCheck: document.getElementById("learningLogCheck"),
  acceptBtn: document.getElementById("acceptBtn"),
  message: document.getElementById("consentMessage")
};
let learner = null;
function getOverrides() { return OneLearnStorage.read("learnerOverrides", {}); }
function saveOverride(userId, patch) {
...
async function init() {
  ...
  const overrides = getOverrides();
  learner = (data.learners || []).map(item => ({ ...item, ...(overrides[item.userId] || {}) })).find(item => item.userId === session.userId);
  if (!learner) throw new Error("학습자 정보를 찾지 못했습니다.");
  const tenant = await OneLearnStorage.fetchJson(`./data/tenants/${tenantId}/tenant.json`);
...
els.acceptBtn.addEventListener("click", () => {
  ...
  if (!els.termsCheck.checked || !els.privacyCheck.checked || !els.learningLogCheck.checked) {
    els.message.textContent = "모든 필수 확인 항목을 체크해주세요.";
    return;
  }
  saveOverride(learner.userId, {
    termsAccepted: true,
    privacyAccepted: true,
    learningLogAccepted: true,
    termsVersion: OneLearnConfig.termsVersion,
    privacyVersion: OneLearnConfig.privacyVersion,
