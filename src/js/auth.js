...
const fallbackLearners = [
  { userId: "usr-001", tenantId: "woongjin", name: "정호준", email: "test@company.com", department: "교육사업팀", employeeNo: "10018177", tempPassword: "10018177", role: "learner", status: "invited", mustChangePassword: true, emailVerified: false, termsAccepted: false, privacyAccepted: false }
];
const els = {
  tenantKicker: document.getElementById("tenantKicker"),
...
function showStep(step) {
  ...
  if (step === "login") {
    ...
    els.stepDesc.textContent = "초기 비밀번호는 안내받은 임시 비밀번호 또는 사번입니다.";
  }
  if (step === "verify") {
  ...
  if (step === "password") {
    ...
    els.stepDesc.textContent = "안전한 계정 관리를 위해 새 비밀번호를 설정합니다.";
  }
}
...
function finishLogin(learner) {
  ...
  if (!nextLearner.termsAccepted || !nextLearner.privacyAccepted) location.href = `./consent.html?tenant=${encodeURIComponent(tenantId)}`;
  else location.href = `./dashboard.html?tenant=${encodeURIComponent(tenantId)}`;
}
...
async function init() {
  try {
    ...
    console.warn("tenant.json 로드 실패. 기본 테넌트를 사용합니다.", error);
    tenant = fallbackTenant;
  }
  try {
    const learnerData = await OneLearnStorage.fetchJson(`./data/tenants/${tenantId}/learners.json`);
    learners = learnerData.learners && learnerData.learners.length ? learnerData.learners : fallbackLearners;
  } catch (error) {
    console.warn("learners.json 로드 실패. 테스트 계정을 사용합니다.", error);
...
els.verifyForm.addEventListener("submit", event => {
  ...
  if (currentLearner.mustChangePassword) showStep("password");
  else finishLogin(currentLearner);
});
