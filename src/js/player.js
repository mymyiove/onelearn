...
/* === v02 PLAYER RESTRICTION RESTORE / HARDENING LAYER ===
   기준: 사용자가 공유한 player.js의 제한 기능 보존 + 우회 경로 보강
   절대 목적: preventForwardSeeking / playbackRate policy / identityCheck / actualWatchSeconds / chapterMovePolicy 유지
*/
let onelearnInternalSeek = false;
let onelearnLastSeekBlockAt = 0;
let onelearnCourseCompletedLogged = false;

function onelearnPolicy(ch){
  return { ...(course?.completionPolicy || {}), ...(ch?.completionPolicy || {}) };
}
function onelearnState(ch){ return state(ch); }
function onelearnDuration(){ return Number(els.video.duration || onelearnState(current).duration || 0); }
function onelearnAllowedTime(){ return Number(onelearnState(current).maxAllowedTime || 0); }
function onelearnCanForwardTo(target){
  const p = onelearnPolicy(current);
  const s = onelearnState(current);
  if(!p.preventForwardSeeking || s.completed) return true;
  return Number(target || 0) <= Number(s.maxAllowedTime || 0) + 1.25;
}
function onelearnBlockedSeek(target, source='unknown'){
  const now = Date.now();
  if(now - onelearnLastSeekBlockAt > 700){
    showToast('아직 학습하지 않은 구간으로 이동할 수 없습니다.');
    logEvent('seek_blocked', { target, source, maxAllowedTime: onelearnAllowedTime() });
    onelearnLastSeekBlockAt = now;
  }
  onelearnInternalSeek = true;
  els.video.currentTime = Math.max(0, Math.min(onelearnAllowedTime(), onelearnDuration() || onelearnAllowedTime()));
  setTimeout(()=>{ onelearnInternalSeek = false; }, 80);
}
function onelearnGuardedSeek(target, source='unknown'){
  target = Math.max(0, Math.min(Number(target || 0), onelearnDuration() || Number(target || 0)));
  if(!onelearnCanForwardTo(target)) return onelearnBlockedSeek(target, source);
  onelearnInternalSeek = true;
...
function applyPlaybackPolicy(){
  ...
  const enabled = Array.from(els.rate.options || []).find(o=>!o.disabled && Number(o.value)===safe);
  if(!enabled){
...
function canMove(t){
  ...
  if(t > idx && p.requireCurrentChapterCompleteBeforeNext && !onelearnState(current).completed){
    return {ok:false,msg:'현재 챕터 완료 후 다음 챕터로 이동할 수 있습니다.'};
  }
  return {ok:true};
}

/* 본인확인: 1회 이상 정책도 분산 체크포인트로 동작 */
function onelearnIdentityCheckpointNeeded(){
  const p = onelearnPolicy(current);
...
function onelearnCheckCourseCompleted(){
  if(onelearnCourseCompletedLogged || !chapters.length) return;
  const allDone = chapters.every(ch=>onelearnState(ch).completed);
  if(allDone){
    onelearnCourseCompletedLogged = true;
    logEvent('course_completed', { completedChapters: chapters.length });
  }
}

/* 핵심 timeupdate 재정의: 실제 시청 시간, 본인확인, 완료 판정 */
els.video.ontimeupdate = ()=>{
  if(!current) return;
  const s = onelearnState(current), d = onelearnDuration(), c = els.video.currentTime || 0, now = Date.now();
  if(d) s.duration = d;
  if(!els.video.paused){
  ...
  if(onelearnIdentityCheckpointNeeded() && !els.identity.classList.contains('hidden')){
    // already open
  }else if(onelearnIdentityCheckpointNeeded()){
    openIdentity();
  }
  if(complete(current)){
  ...
  renderTime();
  renderCourse();
  onelearnCheckCourseCompleted();
};

/* seeking 이벤트 방어: timeline/키보드/브라우저 기본 seek 우회 차단 */
els.video.addEventListener('seeking', ()=>{
  if(onelearnInternalSeek || !current) return;
  const target = Number(els.video.currentTime || 0);
  if(!onelearnCanForwardTo(target)) onelearnBlockedSeek(target, 'video_seeking');
});
...
/* 직접 timeline 클릭도 보호 함수 사용 */
els.timeline.onclick = e=>{
  const rect = els.timeline.getBoundingClientRect();
  const target = ((e.clientX - rect.left) / rect.width) * (onelearnDuration() || 0);
  onelearnGuardedSeek(target, 'timeline');
};

/* 키보드 이동도 제한 로직 경유. 기존 listener가 먼저 움직여도 seeking에서 rollback됨. */
document.addEventListener('keydown', e=>{
  ...
  if(e.key === 'ArrowRight'){
    e.preventDefault();
    onelearnGuardedSeek((els.video.currentTime || 0) + 5, 'keyboard_right');
  }
  if(e.key === 'ArrowLeft'){
    e.preventDefault();
    onelearnGuardedSeek((els.video.currentTime || 0) - 5, 'keyboard_left');
  }
}, true);

/* 사용법 2번: 챕터 드로어 자동 열림, 다른 단계에서 원복 */
